...
续做前比较 DO.md 的证据指纹。HEAD、失败 checks、未解决 issue/MR、`feedback_seen` 之后的新评论、用户约束或开放风险有变化，
...
2. 查 MR 的 CI 状态与反馈。反馈有两种来源，**必须都查**——只查可解决线程会漏掉全部普通评论
   （GitLab 直接发的评论 `resolvable=false`，GitHub 的 PR 对话区评论和 review 总评不在 review threads 里）：
   ...
   - **未解决线程**（人类和 CR 机器人一视同仁）：逐条判断。认可就改、回复"已修：<commit>"并 resolve；
     ...
     人类的线程不设上限。
   - **新普通评论**：创建时间晚于 DO.md 证据指纹里 `feedback_seen` 水位的所有非系统评论（含 ci-review 结论、人类裁决、
     其他 reviewer 意见），排除自己带 `<!-- do-something -->` 标记的回复。同样逐条认可就改、不认可就回复理由；
     普通评论无法 resolve，用一条带标记的回复逐条点名处理结果即闭环。处理完把水位推到本轮开始时读到的最新评论时间。
     指纹里没有水位（首次或旧格式）→ 读全部评论。
   - 反馈处理预计不到半次运行 → 处理完继续选新的一件事；否则它就是本次唯一的事。
3. 全绿、无未解决线程、水位之后无新评论 → 正常走三层判断。
...
# 当前状态
证据指纹：base=<默认分支 sha>；feedback=<CI/线程摘要>；feedback_seen=<已处理到的最新评论 created_at，取 API 原值>；issues=<开放事项摘要>；constraints=<约束摘要>
...
人类通过编辑这个文件来掌舵；MR 模式下 MR 评论是同等的第二信道，第 0 步必须读。
